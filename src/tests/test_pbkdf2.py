...
@pytest.fixture
def pbkdf2_hasher() -> PBKDF2Hasher:
    """
    Fixture for creating a PBKDF2Hasher instance with a specified number of iterations.

    Returns:
        PBKDF2Hasher: An instance of PBKDF2Hasher configured with 100,000 iterations.
    """
    return PBKDF2Hasher(iterations=100_000)


def test_pbkdf2_hash_format(pbkdf2_hasher: PBKDF2Hasher) -> None:
    """
    Test the format of the PBKDF2 hashed string.

    This test ensures that the hashed string generated by the PBKDF2Hasher
    follows the expected format. The format is expected to be:
    'pbkdf2_sha256$iterations$salt$hash'.

    Args:
        pbkdf2_hasher (PBKDF2Hasher): An instance of the PBKDF2Hasher class.
...
def test_pbkdf2_verify_correct_data(pbkdf2_hasher: PBKDF2Hasher) -> None:
    """
    Test the PBKDF2 hasher's ability to verify correct data.

    This test ensures that the PBKDF2Hasher can correctly hash a given
...
def test_pbkdf2_verify_incorrect_data(pbkdf2_hasher: PBKDF2Hasher) -> None:
    """
    Test the PBKDF2Hasher's verify method with incorrect data.
...
def test_pbkdf2_needs_rehash_false(pbkdf2_hasher: PBKDF2Hasher) -> None:
    """
    Test that the PBKDF2Hasher does not require rehashing for a freshly hashed password.
...
def test_pbkdf2_needs_rehash_true(pbkdf2_hasher: PBKDF2Hasher) -> None:
    """
    Test if the PBKDF2 hasher correctly identifies when a hashed password needs rehashing.

    This test creates an instance of PBKDF2Hasher with a lower iteration count (50,000)
    to simulate an outdated hash. It then hashes a sample password and checks if the
    current PBKDF2Hasher instance (with presumably higher iteration count) correctly
    identifies that the old hash needs rehashing.

    Args:
        pbkdf2_hasher (PBKDF2Hasher): The PBKDF2 hasher instance to test against.

    Asserts:
        True if the `needs_rehash` method correctly identifies that the old hash
        needs rehashing.
    """
    old_hasher = PBKDF2Hasher(iterations=50_000)
    old_hashed: str = old_hasher.hash("TestData123!")
    assert pbkdf2_hasher.needs_rehash(old_hashed) is True


def test_pbkdf2_invalid_hash_format(pbkdf2_hasher: PBKDF2Hasher) -> None:
    """
